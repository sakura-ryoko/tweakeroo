...
public class InventoryUtils
{
    ...
    public static void swapHotbarWithInventoryRow(EntityPlayer player, int row)
    {
        ...
        {
            malilib.util.inventory.InventoryUtils.swapSlots(container, slot, hotbarSlot);
            slot++;
        }
    }
    ...
    public static void preRestockHand(EntityPlayer player, EnumHand hand, boolean allowHotbar)
    {
        if (FeatureToggle.TWEAK_HAND_RESTOCK.getBooleanValue() && Configs.Generic.HAND_RESTOCK_PRE.getBooleanValue())
        {
            ...
            malilib.util.inventory.InventoryUtils.preRestockHand(player, hand, threshold, allowHotbar);
        }
    }
    ...
    public static void trySwapCurrentToolIfNearlyBroken(EnumHand hand)
    {
        if (FeatureToggle.TWEAK_SWAP_ALMOST_BROKEN_TOOLS.getBooleanValue())
        {
            ...
            if (ItemWrap.notEmpty(stack) && SWAP_BROKEN_TOOLS_SLOTS.contains(slot))
            {
    ...
    {
        EntityPlayer player = GameWrap.getClientPlayer();
        IBlockState state = GameWrap.getClientWorld().getBlockState(pos);
        ItemStack stack = player.getHeldItemMainhand();

        if (ItemWrap.isEmpty(stack) || stack.getDestroySpeed(state) <= 1f)
        {
            Container container = player.inventoryContainer;
    ...
    private static void swapItemWithHigherDurabilityToHand(EntityPlayer player, EnumHand hand, ItemStack stackReference, int minDurabilityLeft)
    {
        ...
        slotWithItem = malilib.util.inventory.InventoryUtils.findEmptySlotInPlayerInventory(player.inventoryContainer, false, false);
        ...
        for (Slot slot : container.inventorySlots)
        {
            ...
            if (ItemWrap.notEmpty(stack) && stack.getItem().isDamageable() == false)
            {
    ...
    private static void repairModeHandleSlot(EntityPlayer player, EntityEquipmentSlot type)
    {
        ...
        if (slotNum == -1 || ItemWrap.isEmpty(stack))
        {
            return;
        }
    ...
    public static int findSlotWithItem(Container container, ItemStack stackReference, boolean allowHotbar, boolean reverse)
    {
        ...
        for (int slotNum = startSlot; slotNum != endSlot; slotNum += increment)
        {
            ...
            if ((isPlayerInv == false || malilib.util.inventory.InventoryUtils.isRegularInventorySlot(slot.slotNumber, false)) &&
                (allowHotbar || isHotbarSlot(slot) == false) &&
                malilib.util.inventory.InventoryUtils.areStacksEqualIgnoreDurability(slot.getStack(), stackReference))
            {
                return slot.slotNumber;
            }
        }
    ...
    private static void swapItemToHand(EntityPlayer player, EnumHand hand, int slotNumber)
    {
        if (slotNumber != -1 && player.openContainer == player.inventoryContainer)
        {
            ...
            if (hand == EnumHand.MAIN_HAND)
            {
                ...
                {
                    GameWrap.clickSlot(container.windowId, slotNumber, currentHotbarSlot, ClickType.SWAP);
                }
            }
            else if (hand == EnumHand.OFF_HAND)
            {
                ...
                GameWrap.clickSlot(container.windowId, 45, currentHotbarSlot, ClickType.SWAP);
    ...
    private static void swapItemToEqupmentSlot(EntityPlayer player, EntityEquipmentSlot type, int sourceSlotNumber)
    {
        if (sourceSlotNumber != -1 && player.openContainer == player.inventoryContainer)
        {
            ...
            {
                int currentHotbarSlot = player.inventory.currentItem;
                GameWrap.clickSlot(container.windowId, sourceSlotNumber, currentHotbarSlot, ClickType.SWAP);
            }
            else if (type == EntityEquipmentSlot.OFFHAND)
            {
                ...
                // Swap the requested slot to the current hotbar slot
                GameWrap.clickSlot(container.windowId, sourceSlotNumber, tempSlot, ClickType.SWAP);

                // Swap the requested item from the hotbar slot to the offhand
                GameWrap.clickSlot(container.windowId, 45, tempSlot, ClickType.SWAP);
            ...
            else
            {
                ...
                GameWrap.clickSlot(container.windowId, sourceSlotNumber, 0, ClickType.PICKUP);
                // Swap it to the armor slot
                GameWrap.clickSlot(container.windowId, armorSlot, 0, ClickType.PICKUP);
    ...
    private static int findSlotWithSuitableReplacementToolWithDurabilityLeft(Container container, ItemStack stackReference, int minDurabilityLeft)
    {
        for (Slot slot : container.inventorySlots)
        {
            ...
            if (malilib.util.inventory.InventoryUtils.isRegularInventorySlot(slot.slotNumber, false) &&
    ...
    private static void tryCombineStacksInInventory(EntityPlayer player, ItemStack stackReference)
    {
        ...
        for (Slot slot : container.inventorySlots)
        {
            ...
            if (stack.getCount() < stack.getMaxStackSize() && malilib.util.inventory.InventoryUtils.areStacksEqual(stackReference, stack))
            {
                slots.add(slot);
            }
        }
        ...
        for (int i = 0; i < slots.size(); ++i)
        {
            ...
            for (int j = i + 1; j < slots.size(); ++j)
            {
                ...
                if (stack.getCount() < stack.getMaxStackSize())
                {
                    ...
                    GameWrap.clickSlot(container.windowId, slot1.slotNumber, 0, ClickType.PICKUP);
                    GameWrap.clickSlot(container.windowId, slot2.slotNumber, 0, ClickType.PICKUP);

                    // If the items didn't all fit, return the rest
                    if (ItemWrap.notEmpty(player.inventory.getCurrentItem()))
                    {
                        GameWrap.clickSlot(container.windowId, slot1.slotNumber, 0, ClickType.PICKUP);
                    }
    ...
    public static boolean canUnstackingItemNotFitInInventory(ItemStack stack, EntityPlayer player)
    {
        ...
        {
            if (malilib.util.inventory.InventoryUtils.findEmptySlotInPlayerInventory(player.inventoryContainer, false, false) == -1)
            {
                tryCombineStacksInInventory(player, stack);

                return malilib.util.inventory.InventoryUtils.findEmptySlotInPlayerInventory(player.inventoryContainer, false, false) == -1;
            }
        }
            ...
            if (ItemWrap.notEmpty(stack))
            {
                ...
                */
                ...
                if (GameWrap.isCreativeMode())
                {
                    ...
                    GameWrap.getInteractionManager().sendSlotPacket(player.getHeldItem(EnumHand.MAIN_HAND), 36 + inventory.currentItem);
                }
                else
                {
                    ...
                    if (slot != -1)
                    {
                        ...
                        GameWrap.clickSlot(container.windowId, slot, currentHotbarSlot, ClickType.SWAP);
