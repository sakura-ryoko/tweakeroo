...
public class PlacementTweaks
{
    ...
    private static final ItemStack[] stackBeforeUse = new ItemStack[] { ItemStack.EMPTY, ItemStack.EMPTY };
    ...
    public static boolean onProcessRightClickPre(PlayerEntity player, Hand hand)
    {
        ...
        if (FeatureToggle.TWEAK_HAND_RESTOCK.getBooleanValue() &&
                !stackOriginal.isEmpty() &&
            canUseItemWithRestriction(HAND_RESTOCK_RESTRICTION, stackOriginal))
        {
            if (!isEmulatedClick)
            {
                //Tweakeroo.debugLog("onProcessRightClickPre storing stack: {}", stackOriginal);
                cacheStackInHand(hand);
            }

            // Don't allow taking stacks from elsewhere in the hotbar, if the cycle tweak is on
            boolean allowHotbar = !FeatureToggle.TWEAK_HOTBAR_SLOT_CYCLE.getBooleanValue() &&
                    !FeatureToggle.TWEAK_HOTBAR_SLOT_RANDOMIZER.getBooleanValue();
    ...
    {
        //Tweakeroo.debugLog("onProcessRightClickPost -> tryRestockHand with: {}, current: {}", stackBeforeUse[hand.ordinal()], player.getStackInHand(hand));
    ...
    private static void onAttackTick(MinecraftClient mc)
    {
        if (FeatureToggle.TWEAK_FAST_LEFT_CLICK.getBooleanValue())
        {
            ...
            if (mc.player.getAbilities().creativeMode ||
                (Configs.Generic.FAST_LEFT_CLICK_ALLOW_TOOLS.getBooleanValue() || !(mc.player.getMainHandStack().getItem() instanceof MiningToolItem)))
            {
            ...
            BlockHitResult hitResult)
    {
        ...
        if (!canPlaceBlockAgainst(world, posIn, player, hand))
        {
            return ActionResult.PASS;
        }

        //Tweakeroo.debugLog("onProcessRightClickBlock() pos: {}, side: {}, part: {}, hitVec: {}", posIn, sideIn, hitPart, hitVec);
        ...
        if (posFirst == null && result == ActionResult.SUCCESS && restricted)
        {
            ...
            //Tweakeroo.debugLog("plop store @ {}", posFirst);
        }
            ...
            boolean isFirstClick)
    {
        ...
        if (accurate && (accurateIn || accurateReverse || afterClicker))
        {
            ...
            if (accurateIn)
            {
                ...
                //Tweakeroo.debugLog("PlacementTweaks#accurate - IN - facing: {}", facing);
            }
            else if (!flexible || !rotation)
            {
                if (stack.getItem() instanceof BlockItem)
                {
                    ...
                    //Tweakeroo.debugLog("PlacementTweaks#accurate - sideIn: {}, state: {}, hit: {}, posIn: {}, posNew: {}", sideIn, state, hitVec, posIn, posNew);
            ...
            if (accurateReverse)
            {
                //Tweakeroo.debugLog("PlacementTweaks#accurate - REVERSE - facingOrig: {}, facingNew: {}", facing, facing.getOpposite());
            ...
            if ((handleAccurate || afterClicker) && Configs.Generic.CARPET_ACCURATE_PLACEMENT_PROTOCOL.getBooleanValue())
            {
                ...
                //Tweakeroo.debugLog("PlacementTweaks#accurate - pre hitVec: {}", hitVec);
                //Tweakeroo.debugLog("PlacementTweaks#processRightClickBlockWrapper facing: {}, x: {}, posNew: {}, side: {}", facing, x, posNew, side);
                hitVec = new Vec3d(x, hitVec.y, hitVec.z);
                //Tweakeroo.debugLog("PlacementTweaks#accurate - post hitVec: {}", hitVec);
            }

            //Tweakeroo.debugLog("PlacementTweaks#accurate - facing: {}, side: {}, posNew: {}, hit: {}", facing, side, posNew, hitVec);
    ...
    private static boolean canPlaceBlockAgainst(World world, BlockPos pos, PlayerEntity player, Hand hand)
    {
        if (FeatureToggle.TWEAK_PLACEMENT_REST_FIRST.getBooleanValue())
        {
            ...
            if (!stackClickedOn.isEmpty())
            {
                ...
                if (!fi.dy.masa.malilib.util.InventoryUtils.areStacksEqual(stackClickedOn, stack))
                {
                    return false;
                }
            }
        ...
        if (FeatureToggle.TWEAK_PLACEMENT_REST_HAND.getBooleanValue())
        {
            ...
            if (!fi.dy.masa.malilib.util.InventoryUtils.areStacksEqual(stackClicked, stackHand))
            {
                return false;
            }
        }
    ...
    public static void tryRestockHand(PlayerEntity player, Hand hand, ItemStack stackOriginal)
    {
            ...
            canUseItemWithRestriction(HAND_RESTOCK_RESTRICTION, stackOriginal))
        {
            ...
            if (!stackOriginal.isEmpty() && player.getInventory().selectedSlot == hotbarSlot &&
                (stackCurrent.isEmpty() || !ItemStack.areItemsEqual(stackCurrent, stackOriginal)))
            {
                // Don't allow taking stacks from elsewhere in the hotbar, if the cycle tweak is on
                boolean allowHotbar = !FeatureToggle.TWEAK_HOTBAR_SLOT_CYCLE.getBooleanValue() &&
                        !FeatureToggle.TWEAK_HOTBAR_SLOT_RANDOMIZER.getBooleanValue();
            ...
            Hand hand)
    {
            ...
            isFacingValidFor(facing, stackOriginal))
        {
            ...
            //Tweakeroo.debugLog("processRightClickBlockWrapper req facing: {}, x: {}, pos: {}, sideIn: {}", facing, x, posIn, sideIn);
        ...
        //Tweakeroo.debugLog("processRightClickBlockWrapper() pos:{}, side: {}, hitVec: {}", pos, side, hitVec);
        ActionResult result;
        ...
        //Tweakeroo.debugLog("processRightClickBlockWrapper -> tryRestockHand with: {}, current: {}", stackOriginal, player.getStackInHand(hand));
            ...
            world.getBlockState(posPlacement) != stateBefore)
        {
            ...
            {
                //Tweakeroo.debugLog("processRightClickBlockWrapper() after-clicker - i: %d, pos: %s, side: %s, hitVec: %s", i, pos, side, hitVec);
            ...
            @Nullable HitPart hitPart)
    {
        ...
        //Tweakeroo.debugLog("handleFlexibleBlockPlacement() pos: {}, side: {}, facing orig: yaw: {} / pitch: {} facing new: {}", pos, side, yaw, pitch, facing);
