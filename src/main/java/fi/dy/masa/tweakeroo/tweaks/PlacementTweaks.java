...
public class PlacementTweaks {
    ...
    public static void onTick(MinecraftClient mc) {
        ...
        if (use == false) {
            ...
            // Clear the cached stack when releasing both keys, so that the restock doesn't
            // happen when
    ...
    public static boolean onProcessRightClickPre(PlayerEntity player, Hand hand) {
        ...
        if (FeatureToggle.TWEAK_HAND_RESTOCK.getBooleanValue() &&
            ...
            // Don't allow taking stacks from elsewhere in the hotbar, if the cycle tweak is
            // on
    ...
    public static void onProcessRightClickPost(PlayerEntity player, Hand hand) {
        // System.out.printf("onProcessRightClickPost -> tryRestockHand with: %s,
    ...
    private static void onAttackTick(MinecraftClient mc) {
        if (FeatureToggle.TWEAK_FAST_LEFT_CLICK.getBooleanValue()) {
            if (mc.player.getAbilities().creativeMode ||
                    (Configs.Generic.FAST_LEFT_CLICK_ALLOW_TOOLS.getBooleanValue()
                            || (mc.player.getMainHandStack().getItem() instanceof MiningToolItem) == false)) {
        ...
        if (posFirst != null && FeatureToggle.TWEAK_FAST_BLOCK_PLACEMENT.getBooleanValue() &&
            ...
            for (int i = 0; i < maxCount; ++i) {
                ...
                if (hand != null &&
                     ...
                     * 
                     * if (state.getBlock().onBlockActivated(world, posNew, state, player, hand,
                     * side, x, y, z))
                     * {
                     * return;
                     * }
    ...
    public static ActionResult onProcessRightClickBlock(
        ...
        boolean restricted = FeatureToggle.TWEAK_PLACEMENT_RESTRICTION.getBooleanValue()
                || FeatureToggle.TWEAK_PLACEMENT_GRID.getBooleanValue();
        ...
        ActionResult result = tryPlaceBlock(controller, player, world, posIn, sideIn, sideRotated, yaw, hitVec, hand,
                hitPart, true);
    ...
    private static ActionResult tryPlaceBlock(
        ...
        if (flexible) {
            ...
            ItemPlacementContext ctx = new ItemPlacementContext(new ItemUsageContext(player, hand, hitResult));
            posNew = isFirstClick && (rotation || offset || adjacent)
                    ? getPlacementPositionForTargetedPosition(world, posIn, sideIn, ctx)
                    : posIn;
            ...
            if (adjacent && hitPart != null && hitPart != HitPart.CENTER) {
                ...
                hitVec = hitVec
                        .add(Vec3d.of(sideRotatedIn.getOpposite().getVector().add(sideIn.getOpposite().getVector())));
                handleFlexible = true;
            }

            // Place the block facing/against the adjacent block (= just rotated from
            // normal)
            if (rotation) {
                side = sideRotatedIn;
                handleFlexible = true;
        ...
        if (accurate && (accurateIn || accurateReverse || afterClicker)) {
            ...
            if (accurateIn) {
                ...
                // Pistons, Droppers, Dispensers should face into the block, but Observers
                // should point their back/output
                // side into the block when the Accurate Placement In hotkey is used
                if ((stack.getItem() instanceof BlockItem) == false
                        || ((BlockItem) stack.getItem()).getBlock() != Blocks.OBSERVER) {
                ...
                if (stack.getItem() instanceof BlockItem) {
                    ...
                    // System.out.printf("accurate - sideIn: %s, state: %s, hit: %s, f: %s, posNew:
                    // %s\n", sideIn, state, hitVec, EnumFacing.getDirectionFromEntityLiving(posIn,
                    // player), posNew);
            ...
            if ((handleAccurate || afterClicker)
                ...
                double x = hitVec.x;
                int afterClickerClickCount = MathHelper
                        .clamp(Configs.Generic.AFTER_CLICKER_CLICK_COUNT.getIntegerValue(), 0, 32);
                ...
                // System.out.printf("processRightClickBlockWrapper facing: %s, x: %.3f, pos:
                // %s, side: %s\n", facing, x, pos, side);
        ...
        if (handleFlexible) {
            ...
            if (canPlaceBlockIntoPosition(world, posNew, ctx)) {
                // System.out.printf("tryPlaceBlock() pos: %s, side: %s, part: %s, hitVec:
                // %s\n", posNew, side, hitPart, hitVec);
                return handleFlexibleBlockPlacement(controller, player, world, posNew, side, playerYaw, hitVec, hand,
                        hitPart);
            } else {
                return ActionResult.PASS;
            }
        ...
        if (isFirstClick == false && Configs.Generic.FAST_PLACEMENT_REMEMBER_ALWAYS.getBooleanValue()) {
            return handleFlexibleBlockPlacement(controller, player, world, posIn, sideIn, playerYaw, hitVec, hand,
                    null);
        }

        return processRightClickBlockWrapper(controller, player, world, simpleOffset ? posNew : posIn, sideIn, hitVec,
                hand);
    }
    ...
    private static ActionResult processRightClickBlockWrapper(
        ...
        // because this code runs before the cached stack gets set on the first
        // click/use.
        ...
        if (FeatureToggle.TWEAK_PLACEMENT_RESTRICTION.getBooleanValue() &&
            ...
            // FIXME This will break if the block behind the desired position is replaceable
            // though... >_>
        ...
        final int afterClickerClickCount = MathHelper.clamp(Configs.Generic.AFTER_CLICKER_CLICK_COUNT.getIntegerValue(),
                0, 32);
        ...
        boolean rotation = rotationHeld || (rememberFlexible && firstWasRotation);
        boolean accurate = FeatureToggle.TWEAK_ACCURATE_BLOCK_PLACEMENT.getBooleanValue();
        boolean keys = Hotkeys.ACCURATE_BLOCK_PLACEMENT_IN.getKeybind().isKeybindHeld()
        ...
        if (flexible && rotation && accurate == false &&
            ...
            // System.out.printf("processRightClickBlockWrapper req facing: %s, x: %.3f,
        ...
        // System.out.printf("processRightClickBlockWrapper() pos: %s, side: %s, hitVec:
        // %s\n", pos, side, hitVec);
        ActionResult result;
        ...
        if (InventoryUtils.canUnstackingItemNotFitInInventory(stackOriginal, player)) {
            ...
            // System.out.printf("processRightClickBlockWrapper() PLACE @ %s, side: %s, hit:
            // %s\n", pos, side, hitVec);
        ...
        // System.out.printf("processRightClickBlockWrapper -> tryRestockHand with: %s,
        ...
        if (FeatureToggle.TWEAK_AFTER_CLICKER.getBooleanValue() &&
            ...
            for (int i = 0; i < afterClickerClickCount; i++) {
                // System.out.printf("processRightClickBlockWrapper() after-clicker - i: %d,
                // pos: %s, side: %s, hitVec: %s\n", i, pos, side, hitVec);
    ...
    private static ActionResult handleFlexibleBlockPlacement(
        ...
        // System.out.printf("handleFlexibleBlockPlacement() pos: %s, side: %s, facing
    ...
    private static boolean isPositionAllowedByPlacementRestriction(BlockPos pos, Direction side) {
        ...
        PlacementRestrictionMode mode = (PlacementRestrictionMode) Configs.Generic.PLACEMENT_RESTRICTION_MODE
                .getOptionListValue();

        return isPositionAllowedByRestrictions(pos, side, posFirst, sideFirst, restrictionEnabled, mode, gridEnabled,
                gridSize);
    }
    ...
    public static boolean isPositionAllowedByBreakingRestriction(BlockPos pos, Direction side) {
        ...
        if (world != null && FeatureToggle.TWEAK_BLOCK_TYPE_BREAK_RESTRICTION.getBooleanValue()) {
            ...
            if (BLOCK_TYPE_BREAK_RESTRICTION.isAllowed(state.getBlock()) == false) {
                MessageOutputType type = (MessageOutputType) Configs.Generic.BLOCK_TYPE_BREAK_RESTRICTION_WARN
                        .getOptionListValue();

                if (type == MessageOutputType.MESSAGE) {
                    InfoUtils.showGuiOrInGameMessage(Message.MessageType.WARNING,
                            "tweakeroo.message.warning.block_type_break_restriction");
        ...
        PlacementRestrictionMode mode = (PlacementRestrictionMode) Configs.Generic.BREAKING_RESTRICTION_MODE
                .getOptionListValue();

        return posFirstBreaking == null || isPositionAllowedByRestrictions(pos, side, posFirstBreaking,
                sideFirstBreaking, restrictionEnabled, mode, gridEnabled, gridSize);
    }

    private static boolean isPositionAllowedByRestrictions(BlockPos pos, Direction side,
            BlockPos posFirst, Direction sideFirst, boolean restrictionEnabled, PlacementRestrictionMode mode,
            boolean gridEnabled, int gridSize) {
        if (gridEnabled) {
        ...
        if (restrictionEnabled) {
            switch (mode) {
                case COLUMN:
                    return isNewPositionValidForColumnMode(pos, posFirst, sideFirst);
                case DIAGONAL:
                    return isNewPositionValidForDiagonalMode(pos, posFirst, sideFirst);
                case FACE:
                    return isNewPositionValidForFaceMode(pos, side, sideFirst);
                case LAYER:
                    return isNewPositionValidForLayerMode(pos, posFirst, sideFirst);
                case LINE:
                    return isNewPositionValidForLineMode(pos, posFirst, sideFirst);
                case PLANE:
                    return isNewPositionValidForPlaneMode(pos, posFirst, sideFirst);
                default:
                    return true;
            }
        } else {
    ...
    private static BlockPos getPlacementPositionForTargetedPosition(World world, BlockPos pos, Direction side,
    ...
    private static boolean canPlaceBlockIntoPosition(World world, BlockPos pos, ItemPlacementContext useContext) {
        BlockState state = world.getBlockState(pos);
        // boolean isLiquid = state.settings.liquid aka state.liquid -- no call for this
        // exists
    ...
    private static boolean isNewPositionValidForColumnMode(BlockPos posNew, BlockPos posFirst, Direction sideFirst) {
        ...
        switch (axis) {
            case X:
                return posNew.getY() == posFirst.getY() && posNew.getZ() == posFirst.getZ();
            case Y:
                return posNew.getX() == posFirst.getX() && posNew.getZ() == posFirst.getZ();
            case Z:
                return posNew.getX() == posFirst.getX() && posNew.getY() == posFirst.getY();
    ...
    private static boolean isNewPositionValidForDiagonalMode(BlockPos posNew, BlockPos posFirst, Direction sideFirst) {
        ...
        switch (axis) {
            case X:
                return posNew.getX() == posFirst.getX() && Math.abs(relativePos.getY()) == Math.abs(relativePos.getZ());
            case Y:
                return posNew.getY() == posFirst.getY() && Math.abs(relativePos.getX()) == Math.abs(relativePos.getZ());
            case Z:
                return posNew.getZ() == posFirst.getZ() && Math.abs(relativePos.getX()) == Math.abs(relativePos.getY());
    ...
    private static boolean isNewPositionValidForLineMode(BlockPos posNew, BlockPos posFirst, Direction sideFirst) {
        ...
        switch (axis) {
            case X:
                return posNew.getX() == posFirst.getX()
                        && (posNew.getY() == posFirst.getY() || posNew.getZ() == posFirst.getZ());
            case Y:
                return posNew.getY() == posFirst.getY()
                        && (posNew.getX() == posFirst.getX() || posNew.getZ() == posFirst.getZ());
            case Z:
                return posNew.getZ() == posFirst.getZ()
                        && (posNew.getX() == posFirst.getX() || posNew.getY() == posFirst.getY());
     ...
     * {
     * if
     * (InventoryUtils.areStacksEqualIgnoreDurability(player.getHeldItemMainhand(),
     * stackFirst))
     * {
     * return Hand.MAIN;
     * }
     * 
     * if
     * (InventoryUtils.areStacksEqualIgnoreDurability(player.getHeldItemOffhand(),
     * stackFirst))
     * {
     * return Hand.OFF;
     * }
