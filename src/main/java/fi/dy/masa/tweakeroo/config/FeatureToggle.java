...
public enum FeatureToggle implements IHotkeyTogglable, IConfigNotifiable<IConfigBoolean> {
    ...
    TWEAK_ENTITY_REACH_OVERRIDE("tweakEntityReachOverride", false, "",
            "Overrides the entity reach distance with\nthe one set in Generic -> entityReachDistance"),
    ...
    // TODO 1.19.3+
    // TWEAK_CREATIVE_EXTRA_ITEMS ("tweakCreativeExtraItems", false, "", "Adds
    // custom items to item groups.\nSee Lists -> 'creativeExtraItems' to control
    // which items are added to the groups.\nNote: Currently these will be added to
    // the Transportation group\n(because it has the elast items), but in the
    // future\nthe groups will be configurable per added item"),
    // TODO/FIXME 1.19+ the mixin needs an access widener now
    // TWEAK_CUSTOM_FLAT_PRESETS ("tweakCustomFlatPresets", false, "", "Allows
    // adding custom flat world presets to the list.\nThe presets are defined in
    // Lists -> flatWorldPresets"),
    ...
    TWEAK_PERMANENT_SPRINT("tweakPermanentSprint", false, "",
            "If enabled, the player will be always sprinting when moving forward"),
    ...
    TWEAK_SNAP_AIM("tweakSnapAim", false, "", KeybindSettings.INGAME_BOTH,
            "Enabled a snap aim tweak, to make the player face to pre-set exact yaw rotations"),
    ...
    TWEAK_TOOL_SWITCH("tweakToolSwitch", false, "",
            "Enables automatically switching to an effective tool for the targeted block"),
    TWEAK_WEAPON_SWITCH("tweakWeaponSwitch", false, "",
            "Enables automatically switching to a weapon for the targeted entity"),
    ...
    public String getConfigGuiDisplayName() {
        String name = StringUtils.getTranslatedOrFallback("config.name." + this.getName().toLowerCase(),
                this.getName());
    ...
    public String getComment() {
        String comment = StringUtils.getTranslatedOrFallback("config.comment." + this.getName().toLowerCase(),
                this.comment);
    ...
    public void setValueFromJsonElement(JsonElement element) {
        try {
            if (element.isJsonPrimitive()) {
                ...
                Tweakeroo.logger.warn("Failed to set config value for '{}' from the JSON element '{}'", this.getName(),
                        element);
            }
        } catch (Exception e) {
            Tweakeroo.logger.warn("Failed to set config value for '{}' from the JSON element '{}'", this.getName(),
