...
public class InventoryUtils {
    ...
    public static void parseSlotsFromString(String configStr, Collection<Integer> output) {
        ...
        for (String str : parts) {
            try {
                ...
                InfoUtils.showGuiOrInGameMessage(Message.MessageType.ERROR, "Failed to parse slots from string %s",
                        configStr);
            }
        }
    }
    ...
    private static int getSlotNumberForEquipmentType(EquipmentSlot type, @Nullable PlayerEntity player) {
        switch (type) {
            case MAINHAND:
                return player != null ? player.getInventory().selectedSlot + 36 : -1;
            case OFFHAND:
                return 45;
            case HEAD:
                return 5;
    ...
    public static void restockNewStackToHand(PlayerEntity player, Hand hand, ItemStack stackReference,
            boolean allowHotbar) {
        int slotWithItem;

        if (stackReference.getItem().isDamageable()) {
            int minDurability = getMinDurability(stackReference);
            slotWithItem = findSlotWithSuitableReplacementToolWithDurabilityLeft(player.playerScreenHandler,
                    stackReference, minDurability);
        } else {
    ...
    public static void trySwitchToWeapon(Entity entity) {
        ...
        if (player != null && mc.world != null &&
            ...
            if (FeatureToggle.TWEAK_SWAP_ALMOST_BROKEN_TOOLS.getBooleanValue()) {
                test = (currentStack, previous) -> InventoryUtils.isBetterWeaponAndHasDurability(currentStack, previous,
                        entity);
            } else {
                test = (currentStack, previous) -> InventoryUtils.isBetterWeapon(currentStack, previous, entity);
            }

            int slotNumber = findSlotWithBestItemMatch(container, test, UniformIntProvider.create(36, 44),
                    UniformIntProvider.create(9, 35));
    ...
    private static boolean isBetterWeapon(ItemStack testedStack, ItemStack previousWeapon, Entity entity) {
        return testedStack.isEmpty() == false && matchesWeaponMapping(testedStack, entity)
                && (makesMoreDamage(testedStack, previousWeapon)
                        || matchesWeaponMapping(previousWeapon, entity) == false);
    }

    private static boolean isBetterWeaponAndHasDurability(ItemStack testedStack, ItemStack previousTool,
            Entity entity) {
    ...
    public static void trySwitchToEffectiveTool(BlockPos pos) {
        ...
        if (player != null && mc.world != null &&
            ...
            if (FeatureToggle.TWEAK_SWAP_ALMOST_BROKEN_TOOLS.getBooleanValue()) {
                test = (currentStack, previous) -> InventoryUtils.isBetterToolAndHasDurability(currentStack, previous,
                        state);
            } else {
    ...
    private static boolean isBetterToolAndHasDurability(ItemStack testedStack, ItemStack previousTool,
            BlockState state) {
    ...
    private static int findSlotWithBestItemMatch(ScreenHandler container, ItemPickerTest itemTest,
            UniformIntProvider... ranges) {
        final int max = container.slots.size() - 1;
    ...
    private static void swapItemWithHigherDurabilityToHand(PlayerEntity player, Hand hand, ItemStack stackReference,
            int minDurabilityLeft) {
        ScreenHandler container = player.playerScreenHandler;
        int slotWithItem = findSlotWithSuitableReplacementToolWithDurabilityLeft(container, stackReference,
                minDurabilityLeft);
    ...
    private static void repairModeHandleSlot(PlayerEntity player, EquipmentSlot type) {
        ...
        if (stack.isEmpty() == false &&
            ...
            if (slotRepairableItem != -1) {
                ...
                InfoUtils.printActionbarMessage("tweakeroo.message.repair_mode.swapped_repairable_item_to_slot",
                        type.getName());
            }
        }
    }
    ...
    public static void swapElytraWithChestPlate(@Nullable PlayerEntity player) {
        ...
        Predicate<ItemStack> stackFilter = (currentStack.isEmpty() || stackFilterChestPlate.test(currentStack))
                ? stackFilterElytra
                : stackFilterChestPlate;
        Predicate<ItemStack> finalFilter = (s) -> s.isEmpty() == false && stackFilter.test(s)
     ...
     * durability
     * of damageable items. Does not allow crafting or armor slots or the offhand
     * slot
     * in the ContainerPlayer container.
     * 
     * @return the slot number, or -1 if none were found
     */
    public static int findSlotWithItem(ScreenHandler container, ItemStack stackReference, boolean allowHotbar,
            boolean reverse) {
        ...
        for (int slotNum = startSlot; slotNum != endSlot; slotNum += increment) {
            ...
            if ((isPlayerInv == false || fi.dy.masa.malilib.util.InventoryUtils.isRegularInventorySlot(slot.id, false))
                    ...
                    fi.dy.masa.malilib.util.InventoryUtils.areStacksEqualIgnoreDurability(slot.getStack(),
                            stackReference)) {
                return slot.id;
            }
        }
    ...
    private static void swapItemToEquipmentSlot(PlayerEntity player, EquipmentSlot type, int sourceSlotNumber) {
        if (sourceSlotNumber != -1 && player.currentScreenHandler == player.playerScreenHandler) {
            ...
            mc.interactionManager.clickSlot(container.syncId, sourceSlotNumber, equipmentSlotInvIndex,
                    SlotActionType.SWAP, mc.player);
        }
    }
        ...
        if (slotNumber >= 0 && player.currentScreenHandler == player.playerScreenHandler) {
            ...
            if (isHotbarSlot(slotNumber)) {
                ...
                if (PlayerInventory.isValidHotbarIndex(hotbarSlot)) {
                    ...
                    mc.interactionManager.clickSlot(container.syncId, slotNumber, hotbarSlot, SlotActionType.SWAP,
                            mc.player);
                }
            }
        }
    ...
    private static int getUsableHotbarSlotForTool(int currentHotbarSlot, Collection<Integer> validSlots,
    ...
    private static int findSlotWithSuitableReplacementToolWithDurabilityLeft(ScreenHandler container,
            ItemStack stackReference, int minDurabilityLeft) {
    ...
    private static void tryCombineStacksInInventory(PlayerEntity player, ItemStack stackReference) {
        ...
        for (Slot slot : container.slots) {
            ...
            if (stack.getCount() < stack.getMaxCount()
                    && fi.dy.masa.malilib.util.InventoryUtils.areStacksEqual(stackReference, stack)) {
                slots.add(slot);
            }
        }
    ...
    public static boolean canUnstackingItemNotFitInInventory(ItemStack stack, PlayerEntity player) {
        if (FeatureToggle.TWEAK_ITEM_UNSTACKING_PROTECTION.getBooleanValue() &&
            ...
            if (fi.dy.masa.malilib.util.InventoryUtils.findEmptySlotInPlayerInventory(player.playerScreenHandler, false,
                    false) == -1) {
                tryCombineStacksInInventory(player, stack);

                return fi.dy.masa.malilib.util.InventoryUtils.findEmptySlotInPlayerInventory(player.playerScreenHandler,
                        false, false) == -1;
            }
        }
        ...
        if (trace != null && trace.getType() == HitResult.Type.BLOCK) {
            ...
            if (stack.isEmpty() == false &&
                ...
                if (isCreative) {
                    ...
                    mc.interactionManager.clickCreativeStack(player.getStackInHand(Hand.MAIN_HAND),
